...
export default function ContactSubmissionView({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () =>
      adminFetch<ContactSubmission>(`/api/v1/admin/contact-submissions/${id}`, token, {}, logout),
    enabled: !!token,
  });
  ...
  return (
    <div className="admin-page">
      <header className="admin-page-head">
        <h1>Əlaqə #{id}</h1>
        <Link href="/admin/contact-submissions" className="admin-btn admin-btn-ghost">← Geri</Link>
      </header>
      ...
      {data && (
        <div className="admin-detail">
          <DetailRow label="Status">
            <SubmissionStatusSelect resource="contact-submissions" id={data.id} current={data.status} />
          </DetailRow>
